...
const getHighlightItemProps = ({
    ...
    if (!isOver || elementType !== "accordion-item") {
        return {
            top: false,
            bottom: false
        };
...
const AccordionItemsList = () => {
    ...
    const { showConfirmation } = useConfirmationDialog({
        title: "Remove item",
        message: <p>Are you sure you want to remove this accordion item?</p>
    });
    ...
    const onCreate = useCallback(() => {
        updateElement({
            ...element,
            elements: [...element.elements, createElement("accordion-item")]
        });
    }, [element]);

    return (
        <Accordion title={"Accordion Items"} defaultValue={true} className={accordionStyles}>
            <>
                {(element.elements as PbEditorElement[]).map((element, index: number) => (
                    <ListItem
                        key={index}
                        index={index}
                        element={element}
                ...
                <ButtonWrapper>
                    <ButtonSecondary onClick={onCreate}>
                        <ButtonIcon icon={<AddIcon />} /> Add Item
                    </ButtonSecondary>
                </ButtonWrapper>
            </>
        </Accordion>
